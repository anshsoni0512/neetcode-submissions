class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        #     l.append([fre,ele])  #first give freq as sort sees first element of inner list
        # l.sort()  # we got sorted dictonary by frequencies

        # #now get last elemnts from list
        # res=[]
        
        # while len(res)<k:  # dont use equal, if use then if k is 3 then list will have 4 values
...
#         return res
        ...
        # c={}
        # for i in nums:
        #     c[i]=1+c.get(i,0)  #hashmap created

        # freq=[]
        # for i in range(len(nums)+1):
        #     freq.append([])  #creating buckets..

        # # we have value and freq so use items()  and also freq list is ready

        # for ele,count in c.items():
        #     freq[count].append(ele) #inserting ele at appropriate index
